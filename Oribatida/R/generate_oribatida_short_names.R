# Generate short names from Latin Oribatida names
# Script: generate_oribatida_short_names.R
# Purpose: Create 6-letter codes from Oribatida species names (3 letters genus + 3 letters species)
...
# Read the CSV file
oribatida_data <- read_csv("Oribatida_Species_Specializace.csv")
...
oribatida_data$short_name <- sapply(oribatida_data$Species, create_short_name)
# Check for duplicates
duplicates <- oribatida_data %>%
  group_by(short_name) %>%
  summarise(count = n(), .groups = 'drop') %>%
  filter(count > 1)
# Display results
cat("Short names generated!\n")
cat("Number of species processed:", nrow(oribatida_data), "\n")
...
if (nrow(duplicates) > 0) {
  ...
  duplicate_species <- oribatida_data %>%
    filter(short_name %in% duplicates$short_name) %>%
    arrange(short_name)
  print(duplicate_species[, c("Species", "short_name")])
}
# Save the updated data back to the original file
write_csv(oribatida_data, "Oribatida_Species_Specializace_Short_names.csv")
cat("\nShort names column added to original file: Oribatida_Species_Specializace.csv\n")
# Display first few examples
cat("\nFirst 10 examples:\n")
examples <- oribatida_data[1:min(10, nrow(oribatida_data)), c("Species", "short_name")]
print(examples)
